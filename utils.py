...
def not_a_stopword(t):
    return not t.isnumeric() and len(t.strip(' ….!_?')) > 0 and t not in punctuation and t not in arabic_stopwords and t not in english_stopwords and t not in german_stopwords
...
def get_text_normalized(tweet):
    ...
    text = [t for t in text if not_a_stopword(t)]
    # text = [t for t in text if t not in stopwords.words('arabic')]
